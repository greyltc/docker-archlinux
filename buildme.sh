...
sed -i 's,arch-chroot $ROOTFS /bin/sh -c '\''echo $PACMAN_MIRRORLIST > /etc/pacman.d/mirrorlist'\'',install -m755 -D '"$DIR/updateArch.sh"' -t $ROOTFS/usr/sbin/updateArch.sh,g' /tmp/mkimage-arch.sh
